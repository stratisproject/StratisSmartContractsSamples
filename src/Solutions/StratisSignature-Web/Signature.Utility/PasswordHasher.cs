...
namespace Signature.Utility
{
    public static class PasswordHasher
    {
        ...
        private static byte[] ConvertToNetworkOrder(uint number) => BitConverter.GetBytes(number).Reverse().ToArray();

        private static uint ConvertFromNetworkOrder(byte[] reversedUint) => BitConverter.ToUInt32(reversedUint.Reverse().ToArray(), 0);
    }
}
